...
<template>
  ...
  >
    ...
    <a-card :bordered="false" style="background:#f1f1f1">
      ...
      <div><sync-outlined :style="{ color: $config.theme['@primary-color'], marginRight: '8px' }"/> {{ $t('state.inprogress') + ': ' + selectedItems.filter(item => item.status === 'InProgress').length || 0 }}</div>
    </a-card>
    <a-divider />
